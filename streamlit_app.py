...
def display_test_cases(df):
    for index, row in df.iterrows():
        with st.expander(f"**{row['Test Case']}**"):
            ...
            with col2:
                st.write(f"**Test Description:** {row['Test Description']}")
    ...
    if st.session_state.get('show_changes'):
        if st.session_state.changes:
            st.sidebar.markdown("---")
            st.sidebar.header("Recent Changes")
            for change in st.session_state.changes:
                st.sidebar.markdown(f"**{change['id']}** updated:")
                st.sidebar.write(f"  • {change['field']}: '{change['old_value']}' → '{change['new_value']}'")
        
        if st.sidebar.button("Hide Changes"):
            st.session_state.show_changes = False
            st.rerun()
